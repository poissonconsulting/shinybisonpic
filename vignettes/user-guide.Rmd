...
To do this three R packages have been developed to work together to achieve this goal. 
...
There are two apps which are R package that are a user friendly interfaces to [`bisonpictools`](https://github.com/poissonconsulting/bisonpictools). 
...
```
*<font size="-1">Figure x. General layout of the apps.</font>*
...
- **(B)** Enter data into the template by following the rules of the Required data format (box in the top right of Upload Data tab).
  - Description of the Required data format box:
...
```
*<font size="-1">Figure x. Example of the Map Locations tab showing the points on the map with location LOCID11 highlighted as its selected in the drop-down and location LOCID2 clicked on to show a pop-up with the specific details about the location.</font>*
...
- **(D)** At least one value for both the Numerator and Denominator have to be selected for the plot to appear. 

```{r out.width = '100%', echo = FALSE}
knitr::include_graphics("images/shinybisonpic-plot-data-num-den-selected.png")
```
*<font size="-1">Figure x. Example showing male calf selected for the Numerator and female calf selected for the Denominator. Since a value is ticked in both the Numerator and Denominator sections the plot has appeared.</font>*
...
- No plot is appearing.
  ...
  - Confirm that a data set was successfully uploaded by seeing data in the Upload Data tab Uploaded data box.
...
New features or enhancements can also we requested through submitting an [Issue](https://github.com/poissonconsulting/shinybisonpic/issues). 

## runbisonpic

### Getting Started 

This app has to be launched locally from your laptop. 
...
```
*<font size="-1">Figure x. Model tab.</font>*
...
```
*<font size="-1">Figure x. Successful run of the model.</font>*

- **(F)** Download the Analysis Object as an RDS file. 
  - This allows more advanced users to use the output of the model to generate their plots and tables. 
...
New features or enhancements can also we requested through submitting an [Issue](https://github.com/poissonconsulting/runbisonpic/issues).  

## Disclaimer

If the apps are not used as instructed there may be various issues that effect their function. 

If the required data format is not followed there may be issues with the model and output of various widgets. 

The apps have been customized for a specific purpose, if they are used for other purposes they may not function as expected. 
