...
- Enter data into the template file following the rules of the Required data format (box at the top right of the page)
  - Description of the rows in the Required data format box:
    ...
    - joins to: the name of the table the column joins to
      ...
      - for example a location_id of `site_a` must be in the location table to be allowed in the event table
...
```
*<font size="-1">Figure x. Data upload failed due to not following the Required data format, the data must be corrected and uploaded again.</font>*
 
 - Close the pop up box
 - Correct the error
 - Upload the corrected data
 - Repeat until the file is accepted by the app
...
```
*<font size="-1">Figure x. Map Location tab showing the points on the map with location XXXX highlighted as its selected in the drop down and location XXXX clicked on to show a pop-up with the specific details about the location.</font>*

#### 1. Explore camera locations on a map

- The location_id’s from the location table will be displayed on the map
- Explore the points to ensure no data is missing and locations are as expected
...
- Wait a second after you have ticked all the boxes you want to let the plot render
```{r out.width = '100%', echo = FALSE}
...
- The download button is only visible when a plot is present
...
- Click **Download Clean Data** button
